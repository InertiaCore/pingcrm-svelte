...
{#if links && links.length !== 3}
    <div class="mt-6 -mb-1 flex flex-wrap">
        {#each links as { active, label, url } (label)}
            ...
            {:else}
                <Link class="mr-1 mb-1 px-4 py-3 border rounded text-sm hover:bg-white focus:border-indigo-700 focus:text-indigo-700 {active && 'bg-white', label === 'Next' && 'ml-auto'}" href={url}>
                    {label}
                </Link>
            {/if}
        {/each}
